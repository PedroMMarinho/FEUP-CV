...
env_preprocessing = os.getenv("PRE_PROCESSING")
if env_preprocessing:
    PRE_PROCESSING = env_preprocessing.split(",")
else:
    PRE_PROCESSING = []

model_path = "../task_2.0/pool_table_detector_best.pt"
dataset_catalog = "../datasets/task2/parsed_dataset_catalog.json"
catalog = json.load(open(dataset_catalog, "r"))


if "model" in PRE_PROCESSING:
    ...
    for entry in catalog:
        ...
        image_path = "../datasets/" + image_file
        save_path = "pre-preprocessing/" + file_name

        print(f"Processing {image_file}...")
        result = model.predict(image_path, save=False, project=os.path.abspath("./results"), name="run2", exist_ok=True)

        img = cv2.imread(image_path)

        if len(result[0].boxes) > 0:
            # Cut image 
            x1, y1, x2, y2 = max(result[0].boxes, key=lambda box: float(box.conf[0])).xyxy[0].cpu().numpy()
            img = cv2.imread(image_path)
            cropped = img[int(y1):int(y2), int(x1):int(x2)]
            img = cropped
        

        # if "marinho" not in PRE_PROCESSING:
        # Save the cropped image back to the original path
        cv2.imwrite(save_path, img)
        # Change path in catalog
        entry["path"] = "../task2/pre-preprocessing/" + file_name
...
if "marinho" in PRE_PROCESSING:

    images = Path("../datasets/main_dataset/data/train").glob("*.jpg")
    i = 1
    for entry in catalog:

        image_path = "../datasets/" + entry["path"]
        image_name = entry["file_name"]
        save_path = "marinho-processing/" + image_name
        ...
        if final_points is not None:
            ...
            entry["path"] = "../task2/marinho-processing/" + file_name
...
# Store new catalog in dataset_catalog.json
with open("../datasets/task2/parsed_dataset_catalog.json", "w") as f:
